...
pub enum LeagueError {
    ...
    #[error("Duplicate join request: {0}")]
    DuplicateRequest(String),

    #[error("Join request already processed: {0}")]
...
impl IntoResponse for LeagueError {
    fn into_response(self) -> Response {
        let (status, error_message) = match &self {
            ...
            LeagueError::RequestNotFound(msg) => (StatusCode::NOT_FOUND, msg.clone()),
            LeagueError::DuplicateRequest(msg) => (StatusCode::CONFLICT, msg.clone()),
            LeagueError::RequestAlreadyProcessed(msg) => (StatusCode::CONFLICT, msg.clone()),
        };
...
mod tests {
    ...
    #[test]
    fn league_error_duplicate_request_maps_to_409() {
        let error = LeagueError::DuplicateRequest("duplicate".to_string());
        let response = error.into_response();
        assert_eq!(response.status(), StatusCode::CONFLICT);
    }

    #[test]
    fn league_error_request_not_found_maps_to_404() {
        let error = LeagueError::RequestNotFound("req_123".to_string());
        let response = error.into_response();
        assert_eq!(response.status(), StatusCode::NOT_FOUND);
    }

    #[test]
    fn league_error_request_already_processed_maps_to_409() {
        let error = LeagueError::RequestAlreadyProcessed("already done".to_string());
        let response = error.into_response();
        assert_eq!(response.status(), StatusCode::CONFLICT);
    }
}
